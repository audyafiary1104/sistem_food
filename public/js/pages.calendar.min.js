! function(factory) {
    ...
    var PagesCalendar = function(element, options) {
        this.$element = $(element), this.$html = '<div class="calendar">', this.$html += "\x3c!-- START CALENDAR HEADER--\x3e", this.$html += '<div class="calendar-header">', this.$html += '<div class="drager">', this.$html += '<div class="years" id="years"></div>', this.$html += "</div>", this.$html += "</div>", this.$html += '<div class="options"><div style="padding-right: 60px"><row class="pull-right"><div class="clueEvent readonly bg-danger-lighter"><div style="padding:5px 20px 5px 20px" class="event-inner"><div class="event-title checkbox chec check-success"><input checked type="checkbox" class="clueCheckbox" id="bg-danger-lighter"><label for="bg-danger-lighter">Philippines regular holiday</label></div></div></div> &nbsp<div class="clueEvent readonly bg-master-lighter"><div style="padding:5px 20px 5px 20px" class="event-inner"><div class="event-title checkbox chec check-success"><input checked type="checkbox" class="clueCheckbox" id="bg-master-lighter"> <label for="bg-master-lighter">Special non-working day holiday</label></div></div></div> </row> &nbsp <row style="padding-right: 20px" class="pull-right"> <div class="clueEvent readonly bg-success-lighter"><div style="padding:5px 20px 5px 20px" class="event-inner"><div class="event-title checkbox check-success"><input checked type="checkbox" class="clueCheckbox" id="bg-success-lighter"> <label for="bg-success-lighter">Employee Leave Form</label></div></div></div> &nbsp <div class="clueEvent readonly bg-complete-lighter"><div style="padding:5px 20px 5px 20px" class="event-inner"><div class="event-title checkbox  check-success"><input checked type="checkbox" class="clueCheckbox" id="bg-complete-lighter"> <label for="bg-complete-lighter">Calendar Local</label></div></div></div> &nbsp</row></div>', this.$html += '<div class="months-drager drager">', this.$html += '<div class="months" id="months"></div>', this.$html += "</div>", this.$html += '<h4 class="semi-bold date" id="currentDate">&amp;</h4>', this.$html += '<div class="drager week-dragger">', this.$html += '<div class="weeks-wrapper" id="weeks-wrapper">', this.$html += " </div>", this.$html += "</div>", this.$html += "</div>", this.$html += "\x3c!-- START CALENDAR GRID--\x3e", this.$html += '<div id="calendar" class="calendar-container">', this.$html += "</div>", this.$html += "\x3c!-- END CALENDAR GRID--\x3e", this.$html += "</div>", this.$element.append(this.$html);
        var plugin = this;
        plugin.settings = $.extend(!0, {}, $.fn.pagescalendar.defaults, options);
        var EventManager = EventManager || {},
            yearSelector = (EventManager = {
                getEventByID: function(id) {
                    for (var i = 0; i < this.events.length; i++)
                        if (Calendar.settings.events[i]._id == id) return setting.events[i];
                    return null
                },
                ...
                byDateRange: function(startDate, endDate) {
                    for (var start = moment(startDate).format("YYYY-MM-DD"), end = moment(endDate).add(24, "hours").format("YYYY-MM-DD"), range = moment.range(start, end), disableDatesInRange = [], i = 0; i < Calendar.settings.disableDates.length; i++) {
                ...
                return yearSelector.prototype.render = function() {
                    ...
                    $(this.container).append(Calendar.content), Calendar.dragHandler("years"), _setActive(), $(document).off("click", "body:not(.pending) .year-selector"), $(document).on("click", "body:not(.pending) .year-selector", function(e) {
        ...
        monthSelector.prototype = {
            render: function() {
                ...
                for (var months = moment.monthsShort(), currentMonth = moment([calendar.year, calendar.month, calendar.date]).format(Calendar.settings.ui.month.format), i = 0; i < months.length; i++) {
                    ...
                    Calendar.content += '<div class="month">', Calendar.content += '<a href="#" class="month-selector ' + activeClass + '" data-month="' + formatedMonth + '">' + formatedMonth + "</a>", Calendar.content += "</div>"
                }
            ...
            _bindEvents: function() {
                var $this = this;
                $(document).off("click", "body:not(.pending) .month-selector"), $(document).on("click", "body:not(.pending) .month-selector", function(e) {
                    var month = $(this).attr("data-month");
                    calendar.month = moment(month, Calendar.settings.ui.month.format).month(), Calendar.daysOfMonth = moment([calendar.year, calendar.month]).daysInMonth(), Calendar.daysOfMonth < calendar.date && (calendar.date = Calendar.daysOfMonth), Calendar._onMonthChange(), $this._setActive()
                })
            }
        };
        ...
        dateSelector.prototype = {
            render: function() {
                ...
                for (var weekStart = parseInt(moment(Calendar.settings.ui.week.startOfTheWeek, "d").format("d")), weekEnd = parseInt(moment(Calendar.settings.ui.week.endOfTheWeek, "d").format("d")), i = 1; i <= Calendar.daysOfMonth; i++) {
                    ...
                    function _construcWeekday() {
                        Calendar.content += '<div class="day-wrapper date-selector">', Calendar.content += '<div class="week-day">', Calendar.content += '<div class="day week-header">' + date.format(Calendar.settings.ui.week.header.format) + "</div>", Calendar.content += "</div>", Calendar.content += '<div class="week-date ' + activeClass + '">', Calendar.content += '<div class="day"><a href="#" data-date=' + date.format(Calendar.settings.ui.week.day.format) + ">" + i + "</a></div>", Calendar.content += "</div>", Calendar.content += "</div>"
                    }(t == weekStart || 1 == i) && (Calendar.content += '<div class="week ' + activeClass + '">'), weekEnd <= weekStart ? (weekStart <= t || t == weekEnd) && _construcWeekday() : weekStart <= t && t <= weekEnd && _construcWeekday(), t == weekEnd && (Calendar.content += "</div>")
                }
            ...
            _bindEvents: function() {
                $(document).off("click", "body:not(.pending) .date-selector"), $(document).on("click", "body:not(.pending) .date-selector", function(e) {
                    $(".week-date").removeClass("active"), $(this).children(".week-date").addClass("active"), calendar.date = parseInt($(this).children(".week-date").children(".day").children("a").attr("data-date")), Calendar._onDayChange()
                })
            }
        };
        ...
        wView.prototype = Object.create(view.prototype), wView.prototype.render = function() {
            this.weekStart = parseInt(moment(Calendar.settings.ui.week.startOfTheWeek, "d").format("d")), this.weekEnd = parseInt(moment(Calendar.settings.ui.week.endOfTheWeek, "d").format("d")), this.startOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf("week"), this.weekStart >= this.weekEnd ? this.endOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf("week").add("days", 7 + parseInt(Calendar.settings.ui.week.endOfTheWeek)) : this.endOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf("week").add("days", parseInt(Calendar.settings.ui.week.endOfTheWeek)), this._buildLayout(), this._timeslots(), this._loadDates(), this._bindEventDraggers(), this._setActive(), this.windowResize(), Calendar.settings.ui.grid.scrollToFirstEvent && this.scrollToFirstEvent()
            ...
            headerContent += "</div>", $(this.container).html(""), Calendar.content = "", Calendar.content += '<div class="view ' + this.layout + '-view">', Calendar.content += '<div class="allday-cell">', Calendar.content += "</div>", Calendar.content += '<div class="tble" id="viewTableHead">', Calendar.content += headerContent, Calendar.content += "</div>", Calendar.content += '<div class="grid slot-' + Calendar.settings.slotDuration + '">', Calendar.content += '<div class="time-slot-wrapper" id="time-slots">', Calendar.content += "</div>", Calendar.content += '<div class="tble" id="weekGrid">';
            ...
            for (var i = this.weekStart; i <= weekend; i++) {
                ...
                $("#viewTableHead").find(".thead .tcell:nth-child(" + column + ")").html('<div class="weekdate">' + moment(date).format("D") + '</div><div class="weekday">' + moment(date).format("dddd") + "</div>").attr("data-day", moment(date).format("YYYY-MM-DD")), column++
            }
            ...
            for (var height, slotParent = $($(".tble .cell-inner[data-time-slot='" + eventStartHours + ":00']")[cellNo]).parent(), duration = moment(Calendar.settings.events[arrayIndex].end).diff(Calendar.settings.events[arrayIndex].start, "minutes"), remainingMins = duration = Math.round(duration / Calendar.settings.slotDuration) * Calendar.settings.slotDuration; 0 < remainingMins;) remainingMins > this.maxMinsPerDay ? (height = MAXMINS / Calendar.settings.slotDuration * this.snapGridHeight, remainingMins -= MAXMINS, remainingMins = Math.round(remainingMins / Calendar.settings.slotDuration) * Calendar.settings.slotDuration, $(slotParent.children()[slotNumber]).append(this._buildEventElement(height, arrayIndex, cellNo, eventStartHours, eventStartMins, eventEndHours, evenEndMins)), eventStartMins = eventStartHours = 0, cellNo = parseInt(cellNo) + 1, slotParent = $($(".tble .cell-inner[data-time-slot='" + eventStartHours + ":00']")[cellNo]).parent()) : (height = remainingMins / Calendar.settings.slotDuration * this.snapGridHeight, remainingMins = 0, $(slotParent.children()[slotNumber]).append(this._buildEventElement(height, arrayIndex, cellNo, eventStartHours, eventStartMins, eventEndHours, evenEndMins)));
            ...
            var id = "ca_" + moment(Calendar.settings.events[arrayIndex].start).unix() + arrayIndex,
                ...
                eventContent = "<div class='event-container " + Calendar.settings.events[arrayIndex].class + " " + readonly + "' data-event-duration=" + moment(Calendar.settings.events[arrayIndex].end).diff(Calendar.settings.events[arrayIndex].start, "minutes") + " data-index=" + arrayIndex + " data-startTime=" + Calendar.settings.events[arrayIndex].start + " data-endTime=" + Calendar.settings.events[arrayIndex].end + " id=" + id + " data-id=" + id + " data-row=" + parseInt(eventStartHours) + " data-cell=" + cellNo + " style=" + h + ">";
            return eventContent += "<div class='event-inner'>", eventContent += "<div class='event-title'>" + Calendar.settings.events[arrayIndex].title + "</div>", eventContent += "<div class='time-wrap'><span class='event-start-time'>" + moment(Calendar.settings.events[arrayIndex].start).format(Calendar.settings.timeFormat) + "</span> - ", eventContent += "<span class='event-end-time'>" + moment(Calendar.settings.events[arrayIndex].end).format(Calendar.settings.timeFormat) + "</span></div>", eventContent += "</div>", 0 != Calendar.settings.events[arrayIndex].readOnly && null != Calendar.settings.events[arrayIndex].readOnly || (eventContent += "<div class='resizable-handle'></div>"), eventContent += "</div>", Calendar.settings.events[arrayIndex].dataID = id, eventContent
            ...
            for (var i = 0; i < _eventIndex.length; i++) {
                ...
                var eventEndHours, evenEndMins, cellNo = parseInt(dayOfTheWeek) - parseInt(Calendar.settings.ui.week.startOfTheWeek),
                    eventStartHours = e.format("H"),
                    eventStartMins = e.format("m");
                null == Calendar.settings.events[index].end ? (eventEndHours = parseInt(eventStartHours) + 1, evenEndMins = "0") : (eventEndHours = moment(Calendar.settings.events[index].end).format("H"), evenEndMins = moment(Calendar.settings.events[index].end).format("m"));
                var eventDuration = moment(eventEndHours + ":" + evenEndMins, "h:mm").diff(moment(eventStartHours + ":" + eventStartMins, "h:mm"), "minutes", !0);
                Calendar.settings.ui.grid.eventBubble && null != Calendar.settings.events[index].class && this._setEventBubble(Calendar.settings.events[index].class, cellNo), this.renderEvent(eventStartHours, eventStartMins, eventEndHours, evenEndMins, cellNo, index, eventDuration)
            }
            ...
            for (var collisionGroups = [], i = 1; i < _eventIndex.length; i++) {
                ...
                do {
                    var temp = [];
                    event2Range = moment.range(Calendar.settings.events[_eventIndex[j]].start, Calendar.settings.events[_eventIndex[j]].end), event1Range.overlaps(event2Range) && (temp.push(_eventIndex[i]), temp.push(_eventIndex[j]), collisionGroups.push(temp)), j--
                } while (0 === j)
            }

            function union_arrays(array) {
                for (var a = array.concat(), startLength = a.length, i = 0; i < a.length; ++i)
            ...
            for (i = 0; i < collisionGroups.length; i++) {
                ...
                for (j = 0; j < collisionGroups[i].length; j++) {
                    $(".event-container[data-index='" + collisionGroups[i][j] + "']").css({
                        width: width + "%",
                        left: left + "%"
                    }), left += width
                }
            ...
            interact(".event-container:not(.readonly)").draggable({
                ...
                onend: function(event) {
                    ...
                    eventO.readOnly ? $this._drawEvent() : (eventO.start = moment(eventO.start).add(days, "days"), eventO.start = moment(eventO.start).add(mins, "minutes").format(), eventO.end = moment(eventO.start).add(el.attr("data-event-duration"), "minutes").format(), Calendar.settings.events[eventData.index] = eventO, days = mins = 0, event.target.classList.remove("dragging"), $this._drawEvent(), plugin.settings.onEventDragComplete(eventO))
                }
            }).resizable({
                edges: {
                    left: !1,
                ...
                var target = event.target,
                    ...
                    eventO = Calendar.constructEventForUser(eventData.index);
                eventO.readOnly ? $this._drawEvent() : (eventO.end = moment(eventO.start).add(duration, "minutes").format(), days = duration = 0, Calendar.settings.events[eventData.index] = eventO, $this._drawEvent(), plugin.settings.onEventDragComplete(eventO))
            })
            ...
            var d = moment([calendar.year, calendar.month, calendar.date]).format("e"),
                index = 0;
            index = 0 == d && parseInt(Calendar.settings.ui.week.startOfTheWeek) > d ? 7 : parseInt(d) + (1 - parseInt(Calendar.settings.ui.week.startOfTheWeek)), $("#viewTableHead").find(".thead .tcell:nth-child(" + index + ")").addClass("active"), $("#weekGrid").find(".trow .tcell:nth-child(" + index + ")").addClass("active")
            ...
            var elem = $(obj),
                date = $("#viewTableHead").find(".thead .tcell:nth-child(" + (elem.parent().index() + 1) + ")").attr("data-day"),
                time = moment(elem.attr("data-time-slot"), ["H:mm"]).format(" H:mm");
            return {
                date: date = moment(date + time, "YYYY/MM/D h:mm").format()
            }
            ...
            for (var i = 0; i < this.disableDates.length; i++) $(".tble .trow .tcell:nth-child(" + (parseInt(this.disableDates[i]) + 1) + ")").addClass("disable").children(".cell-inner").addClass("disable")
        }, wView.prototype.refresh = function() {
            var currentDate = moment([calendar.year, calendar.month, calendar.date]);
            this.weekStart = parseInt(moment(Calendar.settings.ui.week.startOfTheWeek, "d").format("d")), this.weekEnd = parseInt(moment(Calendar.settings.ui.week.endOfTheWeek, "d").format("d")), 0 == this.weekEnd && (this.weekEnd = 7), 0 == currentDate.weekday() && 0 < this.weekStart ? this.startOfWeek = moment([calendar.year, calendar.month, calendar.date]).subtract("days", this.weekEnd).startOf("week") : this.startOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf("week"), this.endOfWeek = currentDate.startOf("week").add("days", this.weekEnd), this._loadDates(), this._setActive();
            ...
            for (var item in $(".has-event").removeClass("has-event"), Calendar.settings.events) {
                ...
                Calendar.settings.ui.year.eventBubble && $('.year > [data-year="' + eventYear + '"]').addClass("has-event"), calendar.year == moment(Calendar.settings.events[item].start).format("YYYY") && (Calendar.settings.ui.month.eventBubble && $('.month > [data-month="' + eventMonth + '"]').addClass("has-event"), calendar.month + 1 == moment(Calendar.settings.events[item].start).format("M") && Calendar.settings.ui.week.eventBubble && $('.date-selector > .week-date > .day > [data-date="' + eventDate + '"]').addClass("has-event"))
            }
        };
        ...
        return MonthView.prototype = Object.create(view.prototype), MonthView.prototype.render = function() {
            this.weekStart = parseInt(moment(Calendar.settings.ui.week.startOfTheWeek, "d").format("d")), this.weekEnd = parseInt(moment(Calendar.settings.ui.week.endOfTheWeek, "d").format("d")), this.startOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf("week"), this.endOfWeek = moment([calendar.year, calendar.month, calendar.date]), this._buildLayout(), this._loadDates(), this._bindEvents(), this.bindEventDraggers()
            ...
            headerContent += "</div>", $(this.container).html(""), Calendar.content = "", Calendar.content += '<div class="view month-view">', Calendar.content += '<div class="tble" id="viewTableHead">', Calendar.content += headerContent, Calendar.content += "</div>", Calendar.content += '<div class="grid">', Calendar.content += '<div class="tble" id="monthGrid">';
            for (var i = 0; i < 6; i++) {
                Calendar.content += '<div class="trow" >';
                for (j = this.weekStart; j <= this.weekEnd; j++) Calendar.content += '<div class="tcell">', Calendar.content += '<div class="month-date"></div>', Calendar.content += '<div class="cell-inner"><div class="holder"></div></div>', Calendar.content += "</div>";
                Calendar.content += "</div>"
            }
            Calendar.content += "</div>", Calendar.content += "</div>", Calendar.content += "</div>", $(this.container).append(Calendar.content), cellHeight = $(".tcell").innerHeight(), calendar.startOfWeekDate = moment([calendar.year, calendar.month, calendar.date]).startOf("week"), calendar.endOfWeek = moment([calendar.year, calendar.month, calendar.date]).endOf("week").format("D"), this.snapGridWidth = parseInt($(".tcell").outerWidth()), this.snapGridHeight = parseInt($(".cell-inner").outerHeight()), this.holderHeight = parseInt($(".holder").outerHeight()), $(".calendar").removeClass("week").addClass("month")
            ...
            for (var column = 1, i = this.weekStart; i <= this.weekEnd; i++) {
                var date = moment(this.startOfWeek).add(i, "days");
                $("#viewTableHead").find(".thead .tcell:nth-child(" + column + ")").html('</div><div class="weekday">' + moment(date).format("dddd") + "</div>").attr("data-day", moment(date).format("YYYY-MM-DD")), column++
            }
            ...
            for (i = 1; i <= 6; i++)
                for (var j = this.weekStart; j <= this.weekEnd; j++) {
                    ...
                    calendar.month != monthOfDate && (extraClass = "not"), calendar.date == startDate.format("D") && calendar.month == monthOfDate && (extraClass = "current-date active"), this.gridElem.find(".tble .trow:nth-child(" + i + ") .tcell:nth-child(" + (j + 1) + ")").addClass(extraClass).attr("data-date", startDate.format("YYYY-MM-DD")).children(".month-date").html(startDate.format("D")).addClass(extraClass), startDate = startDate.add(1, "days"), extraClass = ""
                }
            ...
            var $this = this;
            $(".month-view .tcell").off("click"), $(".month-view .tcell").on("click", function() {
            ...
            var width = "style='width:" + 100 * parseInt(days) + "%'",
                ...
                readonly = 1 == Calendar.settings.events[arrayIndex].readOnly ? "readonly" : "",
                eventContent = "<div class='event-container " + Calendar.settings.events[arrayIndex].class + " " + readonly + "' " + width + " data-index=" + arrayIndex + " data-startTime=" + Calendar.settings.events[arrayIndex].start + " data-endTime=" + Calendar.settings.events[arrayIndex].end + " id=" + id + " data-id=" + id + ">";
            eventContent += "<div class='event-inner'>", eventContent += "<div class='event-title'>" + Calendar.settings.events[arrayIndex].title + "</div>", eventContent += "</div>", eventContent += "</div>", elem.children(".cell-inner").children(".holder").append(eventContent), Calendar.settings.events[arrayIndex].dataID = id, plugin.settings.onEventRender.call(this)
            ...
            interact(".event-container:not(.readonly)").draggable({
                ...
                event.draggable.snap(!1), event.draggable.snap({
                    anchors: [startPos]
                }), event.target.classList.remove("drop-target"), $(event.target).find(".ghost-element").remove()
                ...
                var eventData = (draggableElement = $(draggableElement)).data(),
                    eventO = Calendar.constructEventForUser(eventData.index);
                eventO.readOnly ? $this._drawEvent() : (eventO.start = moment(eventO.start).set("month", moment(drop.attr("data-date")).get("month")).format(), eventO.start = moment(eventO.start).set("date", moment(drop.attr("data-date")).get("date")).format(), eventO.end = moment(eventO.start).set("date", moment(drop.attr("data-date")).get("date")).format(), Calendar.settings.events[eventData.index] = eventO, $this._drawEvent(), plugin.settings.onEventDragComplete(eventO))
            })
        }, MonthView.prototype._showDisableDates = function() {
            $(".tble .trow .tcell").removeClass("disable").children(".cell-inner").removeClass("disable"), this.disableDates = EventManager.getDisableDateIndexInDateRange(this.monthViewStartDate.format("YYYY-MM-DD"), this.monthViewEndDate.format("YYYY-MM-DD"), "YYYY-MM-DD");
            for (var i = 0; i < this.disableDates.length; i++) $(".tble .trow .tcell[data-date='" + this.disableDates[i] + "']").addClass("disable").children(".cell-inner").addClass("disable")
            ...
            for (var item in $(".has-event").removeClass("has-event"), Calendar.settings.events) {
                var eventYear = moment(Calendar.settings.events[item].start).format(Calendar.settings.ui.year.format),
                    eventMonth = moment(Calendar.settings.events[item].start).format(Calendar.settings.ui.month.format),
                    eventDate = moment(Calendar.settings.events[item].start).format("D");
                $('.year > [data-year="' + eventYear + '"]').addClass("has-event"), calendar.year == moment(Calendar.settings.events[item].start).format("YYYY") && ($('.month > [data-month="' + eventMonth + '"]').addClass("has-event"), calendar.month + 1 == moment(Calendar.settings.events[item].start).format("M") && $('.date-selector > .week-date > .day > [data-date="' + eventDate + '"]').addClass("has-event"))
            }
        }, plugin.Calendar = plugin.Calendar || {}, plugin.Calendar = {
            ...
            _errorMessage: function(msg) {
                $(".grid .tble").prepend("<div class='loading'></div>"), msg && $(".grid .tble .loading").pgNotification({
                    style: "bar",
                    message: msg,
                    position: "top",
                    timeout: 0,
            ...
            dragHandler: function(element) {
                var el = $("#" + element),
                    parent = el.parent();
                if (!$("body").hasClass("mobile") && 1 == el.length) {
                    $(".drager").scrollbar();
                    parent.scrollLeft();
            ...
            scrollToElement: function(el) {
                if (0 == !(el = $(el)).length) {
                    ...
                    if (!this._isElementInViewport(el)) {
                        var offset, elOffset = el.offset().left,
                            elHeight = par.children().width(),
                            windowHeight = $(window).width();
                        offset = elHeight < windowHeight ? elOffset - (windowHeight / 2 - elHeight / 2) : elOffset, $("#weeks-wrapper").parent().animate({
                            scrollLeft: offset
                        }, 10)
                    }
                }
            ...
            nextMonth: function() {
                currentYear = moment([calendar.year, calendar.month, calendar.date]).add(1, "months").year(), currentMonth = moment([calendar.year, calendar.month, calendar.date]).add(1, "months").month(), this._refreshViewOnDateChange()
            },
            previousMonth: function() {
                currentYear = moment([calendar.year, calendar.month, calendar.date]).subtract(1, "months").year(), currentMonth = moment([calendar.year, calendar.month, calendar.date]).subtract(1, "months").month(), this._refreshViewOnDateChange()
            },
            today: function() {
            ...
            constructEventForUser: function(i) {
                return {
                    index: i,
                    id: Calendar.settings.events[i].id,
            ...
            _setDate: function(d) {
                calendar.month = moment(d).month(), calendar.year = moment(d).year(), calendar.date = moment(d).format("D"), calendar.dayOfWeek = moment(d).day(), calendar.monthLong = moment(d).format("MMM"), this._refreshViewOnDateChange()
            },
    ...
    }, PagesCalendar.prototype.destroy = function() {}, PagesCalendar.prototype.setLocale = function(lang) {
    ...
    $.fn.pagescalendar = function(option, obj) {
        ...
        return "string" == typeof option ? data[option](obj) : this.each(function() {
            data || $this.data("pagescalendar", data = new PagesCalendar(this, options))
        })
    }, $.fn.pagescalendar.Constructor = PagesCalendar, $.fn.pagescalendar.defaults = {
        ui: {
            year: {
                visible: !0,
                format: "YYYY",
...
function(root, factory) {
    "function" == typeof define && define.amd ? define(["moment"], function(a0) {
        return root.DateRange = factory(a0)
    }) : "object" == typeof exports ? module.exports = factory(require("moment")) : root.DateRange = factory(moment)
    ...
    function DateRange(start, end) {
        var parts, s = start,
            e = end;
        1 !== arguments.length && void 0 !== end || ("object" == typeof start && 2 === start.length ? (s = start[0], e = start[1]) : "string" == typeof start && (s = (parts = start.split("/"))[0], e = parts[1])), this.start = moment(null === s ? -864e13 : s), this.end = moment(null === e ? 864e13 : e)
    }
    return (DateRange.prototype.constructor = DateRange).prototype.clone = function() {
        ...
        return start <= other.start && other.start < end && end < other.end ? new DateRange(other.start, end) : other.start < start && start < other.end && other.end <= end ? new DateRange(start, other.end) : other.start < start && start <= end && end < other.end ? this : start <= other.start && other.start <= other.end && other.end <= end ? other : null
        ...
        return null === this.intersect(other) ? [this] : other.start <= start && start < end && end <= other.end ? [] : other.start <= start && start < other.end && other.end < end ? [new DateRange(other.end, end)] : start < other.start && other.start < end && end <= other.end ? [new DateRange(start, other.start)] : start < other.start && other.start < other.end && other.end < end ? [new DateRange(start, other.start), new DateRange(other.end, end)] : start < other.start && other.start < end && other.end < end ? [new DateRange(start, other.start), new DateRange(other.start, end)] : void 0
    }, DateRange.prototype.toArray = function(by, exclusive) {
        var acc = [];
        return this.by(by, function(unit) {
            acc.push(unit)
        }, exclusive), acc
    }, DateRange.prototype.by = function(range, hollaback, exclusive) {
        return "string" == typeof range ? function(interval, hollaback, exclusive) {
        ...
        }.call(this, range, hollaback, exclusive) : function(interval, hollaback, exclusive) {
            var div = this / interval,
                l = Math.floor(div);
            if (l === 1 / 0) return;
            l === div && exclusive && l--;
            for (var i = 0; i <= l; i++) hollaback.call(this, moment(this.start.valueOf() + interval.valueOf() * i))
        }.call(this, range, hollaback, exclusive), this
    ...
    }, moment.range.constructor = DateRange, moment.fn.range = moment.range, moment.fn.within = function(range) {
